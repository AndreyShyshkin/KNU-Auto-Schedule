...
public class ScheduleExportService {
    ...
    public byte[] exportToExcel(String title, List<ScheduleEntryDto> schedule) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            String[] columns = {"Day", "Start", "End", "Subject", "Type", "Auditorium", "Teachers/Groups"};
            ...
            for (ScheduleEntryDto entry : schedule) {
                ...
                row.createCell(5).setCellValue(entry.getAuditoriumName());
                row.createCell(6).setCellValue(entry.getAdditionalInfo());
            }
    ...
    public byte[] exportToPdf(String title, List<ScheduleEntryDto> schedule) throws IOException {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            table.setWidths(new float[]{3, 2, 2, 5, 3, 3, 5});

            String[] columns = {"Day", "Start", "End", "Subject", "Type", "Auditorium", "Info"};
