...
export default function ChairView() {
	...
	const [facultyDialogMode, setFacultyDialogMode] = useState<'create' | 'edit'>(
		'create'
	)
	const [facultyFormData, setFacultyFormData] = useState<Partial<Faculty>>({})

	const [openChairDialog, setOpenChairDialog] = useState(false)
	const [chairDialogMode, setChairDialogMode] = useState<'create' | 'edit'>(
		'create'
	)
	...
	const handleDeleteChair = () => {
		if (
			selectedChairId &&
			confirm('Are you sure you want to delete this chair?')
		) {
		...
		<Grid container spacing={2} sx={{ height: '100%' }}>
			...
			>
				...
				>
					...
					<Box>
						<IconButton size='small' onClick={handleAddFaculty}>
							<AddIcon />
						</IconButton>
						<IconButton
							size='small'
			...
			>
				<DialogTitle>
					{facultyDialogMode === 'create' ? 'Add Faculty' : 'Edit Faculty'}
				</DialogTitle>
				<DialogContent>
					<TextField
						autoFocus
			...
			<Dialog open={openChairDialog} onClose={() => setOpenChairDialog(false)}>
				<DialogTitle>
					{chairDialogMode === 'create' ? 'Add Chair' : 'Edit Chair'}
				</DialogTitle>
				<DialogContent>
					<TextField
						autoFocus
						...
						onChange={e =>
							setChairFormData({ ...chairFormData, name: e.target.value })
						}
					/>
					<TextField
						margin='dense'
