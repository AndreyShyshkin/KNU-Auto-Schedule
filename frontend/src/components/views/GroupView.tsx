...
import { 
  fetchSpecialities, fetchGroups, createGroup, updateGroup, deleteGroup,
  Group
...
export default function GroupView() {
  ...
  const updateGroupMutation = useMutation({
    mutationFn: (data: { id: number; group: Partial<Group> }) => updateGroup(data.id, data.group),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['groups'] }),
  });
  ...
  return (
    <Grid container spacing={2} sx={{ height: '100%' }}>
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Typography variant="subtitle1" gutterBottom>Specialities</Typography>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            rows={specialities}
            columns={specColumns}
      ...
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Box>
            <IconButton size="small" disabled={!selectedSpecId} onClick={handleAdd}><AddIcon /></IconButton>
            <IconButton size="small" disabled={!selectedGroupId} onClick={handleEdit}><EditIcon /></IconButton>
            <IconButton size="small" disabled={!selectedGroupId} onClick={handleDelete}><DeleteIcon /></IconButton>
          </Box>
        </Box>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            ...
            density="compact"
            hideFooter
            getRowClassName={(params) => params.row.id === selectedGroupId ? 'Mui-selected' : ''}
            sx={{ '& .Mui-selected': { bgcolor: 'primary.light', '&:hover': { bgcolor: 'primary.light' } } }}
          />
        </Paper>
      </Grid>
