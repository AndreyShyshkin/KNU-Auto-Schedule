...
import { 
  ...
  fetchSubjects, fetchEarmarks, fetchTeachers, fetchGroups,
  Lesson
...
import { 
  ...
  OutlinedInput, Chip, SelectChangeEvent
} from '@mui/material';
...
export default function LessonView() {
  ...
  const { data: earmarks = [] } = useQuery({ queryKey: ['earmarks'], queryFn: fetchEarmarks });
  ...
  const updateLessonMutation = useMutation({
    mutationFn: (data: { id: number; lesson: Partial<Lesson> }) => updateLesson(data.id, data.lesson),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['lessons'] }),
  });
  ...
  return (
    <Grid container spacing={2} sx={{ height: '100%' }}>
      <Grid item xs={12} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Box>
            ...
            <IconButton size="small" disabled={!selectedLessonId} onClick={handleEdit}><EditIcon /></IconButton>
            <IconButton size="small" disabled={!selectedLessonId} onClick={handleDelete}><DeleteIcon /></IconButton>
          </Box>
        </Box>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            ...
            density="compact"
            hideFooter
            getRowClassName={(params) => params.row.id === selectedLessonId ? 'Mui-selected' : ''}
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{dialogMode === 'create' ? 'Add Lesson' : 'Edit Lesson'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                label="Earmark"
                onChange={(e) => setFormData({ ...formData, earmarkId: Number(e.target.value) })}
              >
                {earmarks.map((e) => (
                  <MenuItem key={e.id} value={e.id}>{e.name} ({e.size})</MenuItem>
                ))}
              </Select>
            </FormControl>
            ...
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                onChange={(e: SelectChangeEvent<number[]>) => {
                  ...
                  setFormData({ ...formData, teacherIds: typeof value === 'string' ? value.split(',').map(Number) : value });
                }}
                ...
                renderValue={(selected) => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {selected.map((value) => {
                      ...
                      return <Chip key={value} label={teacher?.name} size="small" />;
                    })}
                  </Box>
                )}
              >
            ...
            <FormControl fullWidth size="small">
              ...
              >
                {groups.map((g) => (
                  <MenuItem key={g.id} value={g.id}>{g.name} ({g.size})</MenuItem>
                ))}
              </Select>
            </FormControl>
