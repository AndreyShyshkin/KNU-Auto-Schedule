...
import { 
  fetchFaculties, fetchSpecialities, createSpeciality, updateSpeciality, deleteSpeciality,
  Speciality
...
export default function SpecialityView() {
  ...
  const createSpecMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['specialities'] }),
  });

  const updateSpecMutation = useMutation({
    mutationFn: (data: { id: number; spec: Partial<Speciality> }) => updateSpeciality(data.id, data.spec),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['specialities'] }),
  });
  ...
  return (
    <Grid container spacing={2} sx={{ height: '100%' }}>
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Typography variant="subtitle1" gutterBottom>Faculties</Typography>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            rows={faculties}
      ...
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Box>
            <IconButton size="small" disabled={!selectedFacultyId} onClick={handleAdd}><AddIcon /></IconButton>
            <IconButton size="small" disabled={!selectedSpecId} onClick={handleEdit}><EditIcon /></IconButton>
            <IconButton size="small" disabled={!selectedSpecId} onClick={handleDelete}><DeleteIcon /></IconButton>
          </Box>
        </Box>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            ...
            density="compact"
            hideFooter
            getRowClassName={(params) => params.row.id === selectedSpecId ? 'Mui-selected' : ''}
            sx={{ '& .Mui-selected': { bgcolor: 'primary.light', '&:hover': { bgcolor: 'primary.light' } } }}
          />
        </Paper>
      </Grid>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        <DialogTitle>{dialogMode === 'create' ? 'Add Speciality' : 'Edit Speciality'}</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
