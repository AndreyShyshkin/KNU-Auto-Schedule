...
import { 
  fetchChairs, fetchTeachers, createTeacher, updateTeacher, deleteTeacher,
  Teacher
...
export default function TeacherView() {
  ...
  const updateTeacherMutation = useMutation({
    mutationFn: (data: { id: number; teacher: Partial<Teacher> }) => updateTeacher(data.id, data.teacher),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['teachers'] }),
  });
  ...
  return (
    <Grid container spacing={2} sx={{ height: '100%' }}>
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Typography variant="subtitle1" gutterBottom>Chairs</Typography>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            rows={chairs}
      ...
      <Grid item xs={6} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          ...
          <Box>
            <IconButton size="small" disabled={!selectedChairId} onClick={handleAdd}><AddIcon /></IconButton>
            <IconButton size="small" disabled={!selectedTeacherId} onClick={handleEdit}><EditIcon /></IconButton>
            <IconButton size="small" disabled={!selectedTeacherId} onClick={handleDelete}><DeleteIcon /></IconButton>
          </Box>
        </Box>
        <Paper sx={{ flexGrow: 1 }}>
          <DataGrid
            ...
            density="compact"
            hideFooter
            getRowClassName={(params) => params.row.id === selectedTeacherId ? 'Mui-selected' : ''}
            sx={{ '& .Mui-selected': { bgcolor: 'primary.light', '&:hover': { bgcolor: 'primary.light' } } }}
          />
        </Paper>
      </Grid>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
        <DialogTitle>{dialogMode === 'create' ? 'Add Teacher' : 'Edit Teacher'}</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
