...
import { 
  fetchTeachers, fetchGroups, fetchTeacherSchedule, fetchGroupSchedule, 
  Teacher, Group, ScheduleEntry 
} from '@/lib/api/scheduleApi';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { 
  Typography, Paper, Grid, Box, FormControl, InputLabel, Select, MenuItem,
...
export default function ScheduleView() {
  ...
  const { data: teachers = [] } = useQuery({ queryKey: ['teachers'], queryFn: fetchTeachers, enabled: mode === 'teacher' });
  const { data: groups = [] } = useQuery({ queryKey: ['groups'], queryFn: fetchGroups, enabled: mode === 'group' });

  const { data: schedule = [], isLoading } = useQuery({
    queryKey: ['schedule', mode, selectedId],
    queryFn: () => mode === 'teacher' 
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'additionalInfo', headerName: mode === 'teacher' ? 'Groups' : 'Teachers', flex: 1 },
  ];
          ...
          <Box sx={{ width: 300 }}>
            {mode === 'teacher' ? (
              <Autocomplete
                ...
                renderInput={(params) => <TextField {...params} label="Select Teacher" size="small" />}
