...
export default function PlacementView() {
	...
	const [selectedEarmarkId, setSelectedEarmarkId] = useState<number | null>(
		null
	)
	...
	const [earmarkDialogMode, setEarmarkDialogMode] = useState<'create' | 'edit'>(
		'create'
	)
	...
	const [auditoriumDialogMode, setAuditoriumDialogMode] = useState<
		'create' | 'edit'
	>('create')
	...
	const handleSubmitEarmark = () => {
		if (earmarkDialogMode === 'create') {
			...
			updateEarmarkMutation.mutate({
				id: selectedEarmarkId,
				earmark: earmarkFormData,
			})
		}
	...
	const handleSubmitAuditorium = () => {
		if (auditoriumDialogMode === 'create') {
			...
			updateAuditoriumMutation.mutate({
				id: selectedAuditoriumId,
				auditorium: auditoriumFormData,
			})
		}
		...
		<Grid container spacing={2} sx={{ height: '100%' }}>
			...
			>
				...
				>
					...
					<Box>
						<IconButton size='small' onClick={handleAddEarmark}>
							<AddIcon />
						</IconButton>
						<IconButton
							size='small'
							disabled={!selectedEarmarkId}
				...
				<Paper sx={{ flexGrow: 1 }}>
					<DataGrid
						...
						getRowClassName={params =>
							params.row.id === selectedEarmarkId ? 'Mui-selected' : ''
						}
						sx={{
							'& .Mui-selected': {
			...
			>
				...
				>
					...
					<Box>
						...
						<IconButton
							size='small'
							disabled={!selectedAuditoriumId}
							onClick={handleEditAuditorium}
						>
							<EditIcon />
						</IconButton>
						<IconButton
				...
				<Paper sx={{ flexGrow: 1 }}>
					<DataGrid
						...
						onRowClick={params =>
							setSelectedAuditoriumId(params.row.id as number)
						}
						density='compact'
						hideFooter
						getRowClassName={params =>
							params.row.id === selectedAuditoriumId ? 'Mui-selected' : ''
						}
						sx={{
							'& .Mui-selected': {
								bgcolor: 'primary.light',
			...
			{/* Earmark Dialog */}
			<Dialog
				open={openEarmarkDialog}
				onClose={() => setOpenEarmarkDialog(false)}
			>
				<DialogTitle>
					{earmarkDialogMode === 'create' ? 'Add Earmark' : 'Edit Earmark'}
				</DialogTitle>
				<DialogContent>
					<TextField
						autoFocus
						...
						onChange={e =>
							setEarmarkFormData({ ...earmarkFormData, name: e.target.value })
						}
					/>
					<TextField
						margin='dense'
			...
			{/* Auditorium Dialog */}
			<Dialog
				open={openAuditoriumDialog}
				onClose={() => setOpenAuditoriumDialog(false)}
			>
				<DialogTitle>
					{auditoriumDialogMode === 'create'
						? 'Add Auditorium'
						: 'Edit Auditorium'}
				</DialogTitle>
				<DialogContent>
