...
export const fetchTeacherSchedule = async (teacherId: number): Promise<ScheduleEntry[]> => {
  const { data } = await axios.get(`/api/schedule/teacher/${teacherId}`);
  return data;
};

export const fetchGroupSchedule = async (groupId: number): Promise<ScheduleEntry[]> => {
